...
irc.addListener('match', function(release, test) {
	if (!cachedirty && sbcache && ((sbcache['lastupdate']+(config.sb.ttl*60)) > Math.round((new Date().getTime())/1000))) {
		logger.debug('Shows cached recently, not going to query SickRage');
		return validateRelease(release, test);
	}

	logger.debug('Cache is too old, going to get a fresh set of data from SickRage');
	cacheShows(function(err) {
		if (err) return logger.error('SickRage error: '+err);
...
function collectShows(callback) {
	sb.listShows(function(err, shows) {
		...
		logger.debug('Received '+total+' shows from SickRage');
...
function validateMatch(show, release, test) {
	logger.info('Found name-match with SickRage: '+release['show']+' - '+release['setext']);
	...
	logger.info('We want that quality ('+quality['display']+'), checking if SickRage already has any episodes');
...
function validateEpisode(show, release, quality, test) {
	sb.getEpisode(show['tvdbid'], release['season'], release['episode'], function(err, episode) {
		...
		if (proper) {
			...
			logger.debug('Release quality is not flagged as archive in SickRage, the episode we already have is good enough');
		}
	});
}
...
function checkTvdbId(show, epNum, release, quality, test) {
	logger.debug('Making sure ID from BTN matches with SickRage...');

	btn.getTorrent(release['torrentid'], function(err, torrent) {
		if (err) return logger.error('BTN API-error: '+err);

		logger.debug('TheTVDB: '+torrent['TvdbID']+' - TVRage: '+torrent['TvrageID']+' - SickRage: '+show['tvdbid']);

		/* Hack: TVRage-fork of SickRage places TVRage IDs in the shows tvdbid */
		if ((show['tvdbid'] != torrent['TvdbID']) && (show['tvdbid'] != torrent['TvrageID']))
			return logger.info('ID from BTN does not match with SickRage, aborting because of false-positive');

		logger.debug('Shows match - going to tell SickRage to scan');
		if (test) {
			cachedirty = true;
			return logger.info('This is where I tell SickRage to scan the episode, if I weren\'t in test-mode! :)');
		}

		if (epNum) {
			logger.info('Starting a single-episode SickRage scan...');
			sb.scanEpisode(show['tvdbid'], release['season'], epNum, quality, function(err) {
				if (err) return logger.error('SickRage error: '+err);

				cachedirty = true;
				logger.info('Successfully commanded SickRage to search for '+release['show']+' - '+release['setext']);

				hooks.notify(logger, 'download');
			});
		}
		else {
			logger.info('Starting a show SickRage backlog scan...');
			sb.backlogShow(show['tvdbid'], release['season'], quality, function(err) {
				if (err) return logger.error('SickRage error: '+err);

				cachedirty = true;
				logger.info('Successfully commanded SickRage to start backlog scanning '+release['show']);

...
function BTNIRC(logger) {
	...
	this.client = require('./../../client/irc')(logger, {
		...
		port: (config.btn.ircwatch.useIrcSSL ? 6697 : 6667),
		secure: config.btn.ircwatch.useIrcSSL,
		iicert: config.btn.ircwatch.ignoreInvalidIrcSSL,
	...
	this.regex = /(.*) \| (s\d+e\d+|season \d+|\d{4}\.\d{2}\.\d{2}) \| (episode|season) \| (\d+) \| (.*) \| (.*) \| (.*) \| (.*) \| (.*) \| (.*) \| (\d+) \| (.*) \| (.*) \| (.*)/mi;
}
...
BTNIRC.prototype.listen = function() {
	...
	this.client.setMasterEvent(function(from, m) {
		...
		if (m == 'hello') {
			...
			self.client.say(from, '!dirty - mark SickRage cache as dirty, forcing a refresh on next announce');
		}
			...
			self.emit('sbdirty');
			self.client.say(from, 'SickRage cache marked as dirty, will be refreshed on next announce');
		}
	});
...
BTNIRC.prototype.match = function(message, test) {
	...
	if (m &&
		...
		var se;
		if (m[2].match(/\d{4}\.\d{2}\.\d{2}/)) {
			release['airdate'] = m[2];
		}
		else if (release['type'] == 'episode') {
		...
		if ((typeof release['season'] !== 'undefined') || (typeof release['airdate'] !== 'undefined')) {
